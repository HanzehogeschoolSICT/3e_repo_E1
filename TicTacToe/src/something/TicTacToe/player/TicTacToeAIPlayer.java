...
public class TicTacToeAIPlayer extends something.Core.player.AIPlayer<TicTacToeBoard> {
    ...
    public int decideMove() {
        ...
        int index = -1;
        int value = isFirst ? Integer.MIN_VALUE : Integer.MAX_VALUE;
        for (int i = 0; i < marks.length; i++) {
            if (marks[i] == Mark.EMPTY) {
                marks[i] = isFirst ? Mark.CROSS : Mark.NOUGHT;
                int newValue = miniMax(marks, 0, !isFirst);
                if ((isFirst && newValue > value) || (!isFirst && newValue < value)) {
                    value = newValue;
                    index = i;
                }
                marks[i] = Mark.EMPTY;
